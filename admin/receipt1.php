...
<body>
	...
	<label style="font-size:20px;">Recibo Oficial</label>
		...
		$query = $connect->query("SELECT * FROM transaction WHERE transaction_id = '$t_id'") or die (mysqli_error());
		$fetch = $query->fetch();
  ...
  $estado = $_GET['estado'];
	...
	<table class="table table-hover" style="background-color:;">
		...
		<?php
			...
			$query = $connect->query("SELECT deposito FROM transaction tra WHERE tra.transaction_id= $t_id  ") or die(mysqli_error());
			...
			<div class="modal-header">
				...
				<form method="post" enctype="multipart/form-data">
					<center>
						<table>
							<tr>
								<td><input type="file" name="product_imagen" required></td>
              </tr>
		...
		if (isset($_POST['add']))
			{
				...
				$code = rand(0,98987787866533499);
							...
							$name2 = $code.$_FILES["product_imagen"] ["name"];
							$type = $_FILES["product_imagen"] ["type"];
							$size = $_FILES["product_imagen"] ["size"];
							$temp = $_FILES["product_imagen"] ["tmp_name"];
							$error = $_FILES["product_imagen"] ["error"];
			...
			$q2 = $connect->query("UPDATE transaction SET deposito='$name2' WHERE transaction_id='$t_id' ");
