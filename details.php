...
<div id="container">
	<div class="nav">
		...
		<?php
			if(isset($_GET['id'])){
			$id = $_GET['id'];
			$query = $connect->query("SELECT * FROM producto WHERE idproducto = '$id' ");
			$row = $query->fetch();

			$query1 = $connect->query("SELECT * FROM stock WHERE idproducto = '$id'");
			$rows = $query1->fetch();
			$qty = $rows['cantidad'];
		...
		?>
				<div>
					<center>
						<img class="img-polaroid" style="width:; height:;" src="photo/<?php echo $row['imagen']; ?>">
						<h2 class="text-uppercase bg-primary"><?php echo $row['producto']?></h2>
						<h3 class="text-uppercase">PVP:  <?php echo $row['precio']?></h3>

						<h3 class="text-uppercase">Disponibles: <?php echo $qty ?></h3>


						<?php echo "<a href='cart.php?id=".$id."&action=add'><input type='submit' class='btn btn-inverse' name='add' value='Agregar al Carrito'></a> &nbsp;    <a href='product1.php'>   <button class='btn btn-inverse'>Atráz</button></a> " ?>
			...
			<div class="modal-header">
				...
				<div class="modal-body">
					...
					<center>
						...
						<input type="hidden" value="<?php echo $fetch['nombre'];?>&nbsp;<?php echo $fetch['apellido'];?>" name="customer">
						<textarea name="destination" placeholder="Destination" style="height:100px; width:250px;" required></textarea>

						<br />
						<h4>Total: P <?php echo $row['precio']; ?> </h4>
						<br />
						<input type="checkbox" required> I Agree the <a href="#terms" data-toggle="modal"> Terminos y Condiciones</a> de Faben Mobiliaria Inc.
					</center>
				</div>
			<div class="modal-footer">
				<center>
		...
		<div id="terms" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="width:400px;">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
				<h3 id="myModalLabel">Faben Mobiliaria Inc. Terms and Condition</h3>
			</div>
				<div class="modal-body">
					<ul>
