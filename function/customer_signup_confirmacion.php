<?php
  ...
  $query3 = $connect->query("SELECT  cli.nombre,cli.apellido,cli.email FROM venta tra INNER JOIN clientes cli on tra.idcliente=cli.idcliente WHERE tra.idventa='$t_id'   ") or die (mysqli_error());
  $row3 = $query3->fetch();
  $nombre = $row3['nombre'];
...
$feed = 'http://localhost/1/alphaware/admin/factura_email.php'; /* Insert URL here */
$data = my_curl_fun($feed);
  ...
  // $mail_body =  "Link de activacion - ".$base_url."email_verification.php?email=".$email." ";
   ...
   $mail->Subject = "MENSAJE DE CONFIRMACION DE PAGO ultimo";
